...
/// Fetch odds from letsaiabt365.com (or custom domain) API
class OddsService {
  ...
  static Future<List<OddsMatch>> fetchOdds({bool liveOnly = false}) async {
    ...
    final url = 'https://${settings.bwbDomain}/_view/ParGenWFH.ashx?g=2&ot=$ot&wd=&ia=0&update=true';
    ...
    try {
      final resp = await http.get(
        ...
        headers: {
          ...
          'Referer': 'https://${settings.bwbDomain}/',
        },
      );
  ...
  /// Based on BWB365 winning pattern (29/29 legs WIN)
  static List<ParlayPick> generatePicks(List<OddsMatch> matches, {int maxLegs = 10}) {
    final picks = <ParlayPick>[];
    final usedMatches = <String>{};

    // Priority 1: O/U Under in defensive leagues (Swedish, Belarus)
    // Priority 2: O/U Over in high-scoring leagues (China, Chile, Brazil)
    // Priority 3: AH small (-0.5, -1.5)
    // Priority 4: 1X2 underdog (>2.0)

    // Sort: best odds first within each category
    ...
    for (final m in matches) {
      ...
      // O/U picks (most reliable per BWB)
      if (m.ouOver >= 1.5 && m.ouOver <= 2.3) {
        ouPicks.add(ParlayPick(
          match: m,
          pick: 'Over 2.5',
          odds: m.ouOver,
          market: 'O/U',
          confidence: m.ouOver < 1.9 ? 'HIGH' : 'MED',
        ));
      }
      if (m.ouUnder >= 1.5 && m.ouUnder <= 2.3) {
        ouPicks.add(ParlayPick(
          ...
          odds: m.ouUnder,
          market: 'O/U',
          confidence: m.ouUnder < 1.9 ? 'HIGH' : 'MED',
        ));
      }

      // AH small only
      if (m.ahHome >= 1.5 && m.ahHome <= 2.2 && m.ahLine.abs() <= 0.75) {
        ahPicks.add(ParlayPick(
          match: m,
          pick: 'AH Home ${m.ahLine}',
          odds: m.ahHome,
          market: 'AH',
          confidence: m.ahLine.abs() <= 0.5 ? 'HIGH' : 'MED',
        ));
      }

      // 1X2 underdog only (not favorites <1.50)
      if (m.x12Home >= 2.0 && m.x12Home <= 2.5) {
        x12Picks.add(ParlayPick(
          match: m,
          pick: '1X2 Home',
          odds: m.x12Home,
          market: '1X2',
    ...
    // Build parlay: 5 O/U + 3 AH + 2 1X2 = 10 legs
    for (final p in ouPicks) {
      final key = '${p.match.home}_${p.match.away}';
      if (!usedMatches.contains(key) && picks.length < 5) {
        usedMatches.add(key);
        picks.add(p);
      }
    }
    for (final p in ahPicks) {
      final key = '${p.match.home}_${p.match.away}';
      if (!usedMatches.contains(key) && picks.length < 8) {
        usedMatches.add(key);
        picks.add(p);
      }
    }
    for (final p in x12Picks) {
      final key = '${p.match.home}_${p.match.away}';
      if (!usedMatches.contains(key) && picks.length < maxLegs) {
        usedMatches.add(key);
        picks.add(p);
      }
    }
...
class OddsMatch {
  ...
  final String home;
  final String away;
  final double x12Home;
  final double x12Draw;
  final double x12Away;
  final double ahLine;
  final double ahHome;
  final double ahAway;
  final double ouOver;
  final double ouUnder;
  ...
  OddsMatch({
    ...
    required this.home,
    required this.away,
    required this.x12Home,
    required this.x12Draw,
    required this.x12Away,
    required this.ahLine,
    required this.ahHome,
    required this.ahAway,
  ...
  factory OddsMatch.fromArray(List<dynamic> m, String leagueName) {
    ...
    return OddsMatch(
      league: leagueName,
      time: m.length > 4 ? m[4].toString() : '',
      home: m.length > 6 ? m[6].toString() : '',
      away: m.length > 7 ? m[7].toString() : '',
